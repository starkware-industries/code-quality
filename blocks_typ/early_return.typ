...
#block_header(9, [Early return])\
// Fill text in the 'what' section
#what If without an else = blockchain without decentralization. Technically possible, but mostly useless.\
#why  Your main logic shouldn’t be wrapped in layers of `if`.
#bad_code[```rust
...
\ #how Just flip the condition, add an early return and let the main logic breath.
#good_code[```rust
fn watch_movie(user: User) -> Result<(), WatchMovieError> {
    if !user.is_authenticated() {
...
#tip[
  ...
  #code[```rust let AuthStatus::Authenticated(user) = auth else {
