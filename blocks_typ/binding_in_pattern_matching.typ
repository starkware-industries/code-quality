...
```]
A tuple or a pattern inside a tuple!
#good_code[```rust
if let Some(point @ (x @ 1..i32::MAX, y @ 1..i32::MAX)) = pair {
        println!("{Point} is in the first quadrant with
        x = {x} and y = {y}");
    }
}
```]
A struct field!
#good_code[```rust
if let User { age: age @ 18..=30, .. } = user {
    println!("{} is a young user with age {age}", user.name);
}
```]
