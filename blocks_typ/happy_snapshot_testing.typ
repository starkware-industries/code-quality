...
#what
\ Previously on Block by Block... the happy side of snapshot testing.
\ But - there’s a dark side: *Snapshot tests fail* - not just one, sometimes ten, sometimes hundreds.
\ And the sad news - it’s your job to fix them. Would you like to get some help?
\ #why
...
#how
\ All rise for the *expect-test* crate 🎉  — from now on, hard-coded values fix themselves with this magic fix command: ```rust UPDATE_EXPECT=1 cargo test```.
\ Instead of:
