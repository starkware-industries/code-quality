...
#what
\ This is the question you want to avoid when people read your code.

// Fill text in the 'why' section
#why
\ Code is a guide, not just a mechanism. \
It should show the reader what’s happening and why — not just make the machine run correctly.
...
```]
\ It reads: "Is the user has a write permission or they are performing a read operation""

#code[```py
❌  if not has_write_permissions: assert is_read_operation, "No write permissions."
```]
\ It reads: "If the user doesn't have write permissions, check that the operation is read".

#code[```py
✅ if not has_write_permissions:
   assert is_read_operation, "No write permissions."
```]
\ It reads: "If the user is performing a write operation, we require that they have write permissions".
