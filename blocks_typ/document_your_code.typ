...
#why
\ It gives people the option to understand your code without digging through the details.
...
fn calculate_price(user: User, order: Order) -> u64 {
    ...
    let mut price = order.items.iter().map(|i| i.price).sum();

    // Apply discount if the user is eligible.
    price -= calculate_discount(&user, price);

    // Add VAT according to the user's profile.
    price + calculate_vat(&user, price)
}
```]
// If you need a tip section, fill here
#tip[
  1. Remember - the code usually *doesn't* document itself, at least for the average human. Make sure you document your structs/enums, their members and functions.
  2. Put effort into *naming* your variables and functions. Stuck? Ask your good-at-names teammate for inspiration or your favourite AI tool for inspiration. If it feels impossibly hard - that might be a design problem in disguise.
]
#call_for_action[
